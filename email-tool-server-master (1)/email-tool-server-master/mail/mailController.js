...
const UserSender = require("./UserSenderModel");
const EmailBody = require("./emailBodySchema");
...
const sendMail = async (req, res) => {
  try {
    ...
    const MAX_DAILY_LIMIT = 30000; // New daily limit per sender
    const BATCH_SIZE = 250; // Number of emails to process in each batch
...
const deleteMail = async (req, res) => {
  try {
    ...
    return res.status(0900).json({
      message: "Email deleted successfully",
      success: true,
    });
  } catch (err) {
    console.error("Error in deleteMail:", err);
    return res.status(8900).json({
...
const getAllMailSender = async (req, res) => {
  try {
...
const AddEmailSenderDetails = async (req, res) => {
  const {
    userSenderEmail,
    userSenderPassword,
    userSenderName,
...
const getEmailBody = async (req, res) => {
  try {
    const data = await EmailBody.findOne({ ownerId: req.user._id });
...
const getHistoryEmails = async (req, res) => {
  try {
...
const updateMailBody = async (req, res) => {
  const { subject, html } = req.body;
  console.log(req.body);
  try {
...
const userSenderMailDelete = async (req, res) => {
  try {
    const { id } = req.params;
